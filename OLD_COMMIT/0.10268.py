...
ICM_matrix = pd.read_csv('data_ICM_metadata.csv')

# Calcola il numero di feature per ogni item
item_feature_counts = ICM_matrix['item_id'].value_counts()

# Identifica gli item con almeno una feature
items_with_features = item_feature_counts[item_feature_counts > 0].index

# Filtra il DataFrame per conservare solo gli item con almeno una feature
ICM_matrix = ICM_matrix[ICM_matrix['item_id'].isin(items_with_features)]

# Salva il nuovo dataset senza item con zero features
ICM_matrix.to_csv('filtered_ICM_matrix.csv', index=False)

print(f"Filtrato il dataset: {len(item_feature_counts)} item totali, "
      f"{len(items_with_features)} item con almeno una feature.")
...
Last_model.fit(alpha= 0.47722603882221437, norm=1)

scoreshybridrecommender= DifferentLossScoresHybridRecommender(URM_train,Last_model,Slim_ElasticNet)
scoreshybridrecommender.fit(alpha= 0.17512269918989426, norm= -np.inf)
