...
recommender_ItemKNNCF.fit(topK= 5, shrink= 473, similarity= 'cosine', normalize= True, feature_weighting= 'TF-IDF')
...
RP3_rec.fit(alpha= 0.5617704871506491, beta= 0.34880636236362156, min_rating= 1, topK=  19, implicit= True, normalize_similarity= True)
...
alpha_par = 0.4670392568786762
new_similarity = (
alpha_par * P3alpha.W_sparse +
    (1-alpha_par) * RP3_rec.W_sparse 
     
)

recommender_instance.fit(new_similarity)
print("knncustom p3alpha e rp3beta:")
...
class ScoresHybridRecommender(BaseRecommender):
    ...
    def fit(self, alpha = 0.5, beta=0.5, gamma=0, theta=0.2, omega=0.1):
        self.alpha = alpha      
        self.beta = beta
...
scoreshybridrecommender = ScoresHybridRecommender(URM_train,recommender_object, prova_ease)
#print("theta=", theta_par)
...
"""
recommender_knn2 = ItemKNNCustomSimilarityRecommender(URM_train)
    alpha_par =  3.0172123735021463
    beta_par =  9.089252479218183

    new_similarity = (
        alpha_par * P3alpha.W_sparse +
        beta_par * RP3_rec.W_sparse
    )

    recommender_knn2.fit(new_similarity)
